...
<template>
  <div @paste.prevent="handlePaste" class="grid grid-cols-2 gap-1">
    <div>
      <h1 class="text-xl font-bold">Home Page</h1>
      <div>
        <button><i class="fas fa-list"></i> Prioritize</button>
        <button @click="theStore.createDefault"><i class="fas fa-eraser"></i> Clear</button>
      </div>
      <table>
      ...
      <div>
        ...
        <pre v-if="showDebug">{{ JSON.stringify(theStore.state, null, "  ") }}</pre>
      </div>
    </div>
    <div>
